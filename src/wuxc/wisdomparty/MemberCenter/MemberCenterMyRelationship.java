...
public class MemberCenterMyRelationship extends Activity
	...
	List<MyrelationshipModel> list = new ArrayList<MyrelationshipModel>();
	private static MyrelationshipAdapter mAdapter;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.member_center_myrelationship);
		initview();
		setonclicklistener();
		setheadtextview();
		GetData();
	...
	private void getdatalist(int arg) {
		...
		try {
			...
			for (int i = 0; i < 10; i++) {
				...
				MyrelationshipModel listinfo = new MyrelationshipModel();
				listinfo.setOut("三门峡某组织");
				listinfo.setIn("三门峡某组织");
				listinfo.setTime("2016-11-03");
				if ((arg - 1) * 10 + i == 0) {
					listinfo.setIsPink(true);
				} else {
					listinfo.setIsPink(false);
				}
				list.add(listinfo);
	...
	protected void go() {
		ListData.setPadding(0, -100, 0, 0);
		mAdapter = new MyrelationshipAdapter(this, list, ListData);
