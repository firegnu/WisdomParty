...
package wuxc.wisdomparty.WaterFallFlow;
...
public abstract class PLA_AbsListView extends PLA_AdapterView<ListAdapter>
	...
	public PLA_AbsListView(Context context) {
		...
		try {
			...
			Method initializeScrollbars = android.view.View.class.getDeclaredMethod("initializeScrollbars",
					TypedArray.class);
	...
	protected float getBottomFadingEdgeStrength() {
		...
		if (count == 0) {
			...
			return bottom > height - getPaddingBottom() ? (float) (bottom - height + getPaddingBottom()) / fadeLength
					: fadeEdge;
		}
	}
	...
	private class FlingRunnable implements Runnable {
		...
		void start(int initialVelocity) {
			...
			if (DEBUG)
				Log.d(TAG, String.format("String Fling: [%d, %d] to [%d]", initialY, initialVelocity,
						mScroller.getFinalY()));
